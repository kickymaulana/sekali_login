...
const submit = () => {
  form.post(route('admin.users.store'), {
    onSuccess: () => Snackbar.success('User baru berhasil ditambahkan'),
    onError: () => {
...
<template>
  <Head title="Tambah User - SSO Admin" />
  ...
  <div class="android-layout">
    <header class="top-app-bar">
      ...
      <h1 class="app-bar-title">Tambah User</h1>
    ...
    <main class="android-content">
      <div class="hero-card">
        <div class="hero-text">
          <h3>User Baru 👤</h3>
          <p>Daftarkan pengguna baru untuk mengakses layanan SSO.</p>
        </div>
        <var-icon name="account-plus" class="hero-icon" />
      </div>
      ...
      <div class="form-container">
        <form @submit.prevent="submit">
          ...
          <var-input type="password" v-model="form.password" label="Password" placeholder="Minimal 8 karakter" :error-message="form.errors.password">
            <template #prepend-icon><var-icon name="lock-outline" color="#6366f1" /></template>
          </var-input>

          <var-input type="password" v-model="form.password_confirmation" label="Konfirmasi Password" placeholder="Ulangi password">
