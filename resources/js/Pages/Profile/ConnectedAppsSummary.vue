...
const openTokens = (clientId: string) => {
  router.get(route('aplikasi-terhubung.tokens', clientId))
}
</script>
...
<template>
  ...
  <div class="layout">
    ...
    <main class="content">
      ...
      <div v-if="!props.apps.length" class="empty">
        <var-icon name="apps-box" :size="48" color="#cbd5e1" />
        <p>Belum ada aplikasi terhubung</p>
      </div>

      <div v-for="app in props.apps" :key="app.client_id" class="card">
        <div class="app-info">
          <div class="app-icon">
        ...
        <var-button size="small" type="primary" text @click="openTokens(app.client_id)">Lihat Token</var-button>
      </div>
